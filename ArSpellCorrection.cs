...
namespace ArSpellChecker
{
    public class ArSpellCorrection
    {

        static void Main(string[] args)
        {

            string filename = "Arabic spell-checking dictionary.txt";
            String TXTRes = Console.ReadLine();
            CreateDictionary(filename, "ar");
            string[] tokens = TXTRes.Split(' ');
            string result = "";
            int num; bool isNum;
            string thecorrect = "";
            foreach (string token in tokens) // Tokenization Step
            {
                isNum = Int32.TryParse(token, out num);
                if (token.Equals("،") || token.Equals(".") || token.Equals(":") || token.Equals("؛") || token.Equals("...") || token.Equals("-") || token.Equals("*") || token.Equals("/") || token.Equals("\\")
                || token.Equals("!") || token.Equals("؟") || token.Equals("..") || token.Equals("(") || token.Equals(")") || isNum)
                    result = result + token + " ";
                else
                {
                    thecorrect = ReadFromStdIn(token);
                    if (thecorrect.Equals(""))
                        result = result + token + " ";
                    else
                    {
                        if (token.Contains(":")) thecorrect = thecorrect + ":";
                        result = result + thecorrect + " ";
                    }
            ...
            Console.WriteLine(result.Substring(0, indx - 1) + ".");
 ...
 /// ----------------------------------- Arabic SPELL CHECKER ------------------------
       ...
       sort: suggestions = suggestions.OrderBy(c => c.distance).ThenByDescending(c => c.count).ToList();
        ...
        private static string Correct(string input, string language)
        {
            ...
            */
            ...
            foreach (var suggestion in suggestions)
            {
                ...
                correctedstring = correctedstring + suggestion.term;
                // System.Diagnostics.Debug.WriteLine(" The correct String:  " + suggestion.term);
            }
        ...
        {
            System.Diagnostics.Debug.WriteLine("   I'm NOUF:   ");
            string str = Correct(word, "ar");
            return str;
        ...
        public static Int32 DamerauLevenshteinDistance(String source, String target)
        {
            ...
            return H[m + 1, n + 1];
        } // end DamerauLevenshteinDistance
   
   
         /// ----------------------------------- END Arabic SPELL CHECKER PART ------------------------
        /// ----------------------------------- ------------------- ------------------------
    } // end class
} // end name Space
